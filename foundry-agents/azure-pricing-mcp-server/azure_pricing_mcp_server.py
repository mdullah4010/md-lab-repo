...
try:
    # Import custom logging configuration (consistent with indexer.py)
    from logging_config import get_logger
    logger = get_logger(__name__)
    
    logger.info("🚀 Azure Pricing MCP Server logger initialized successfully with custom logging config")
    ...
    logger.warning(f"⚠️  Using fallback logging configuration due to import error: {e}")
except Exception as e:

    # Fallback logging setup
    logging.config.dictConfig(logging_config)
    logger = logging.getLogger(__name__)
    logger.warning(f"⚠️  Using fallback logging configuration due to error: {e}")
...
if settings.MCP_DEBUG:
    ...
    logger.debug("DEBUG mode activated for Azure Pricing MCP Server")

def log_function_call(func_name: str, **kwargs):
    """Log function calls with their parameters."""
    params_str = ", ".join([f"{k}={repr(v)}" for k, v in kwargs.items()])
    logger.info(f"🔧 FUNCTION CALL: {func_name}({params_str})")

def log_function_result(func_name: str, result: Dict[str, Any], execution_time: float = None):
    """Log function results with key metrics."""
    status = result.get("status", "unknown")
    count = result.get("count", result.get("total_products", 0))
    
    if execution_time:
        logger.info(f"✅ FUNCTION RESULT: {func_name} -> status={status}, count={count}, time={execution_time:.2f}s")
    else:
        logger.info(f"✅ FUNCTION RESULT: {func_name} -> status={status}, count={count}")
    
    # Log additional details in debug mode
    if settings.MCP_DEBUG and isinstance(result, dict):
        logger.debug(f"🔍 DETAILED RESULT for {func_name}: {result}")

def log_api_call(url: str, params: dict, response_status: int = None, response_time: float = None):
    """Log API calls with parameters and response info."""
    params_str = ", ".join([f"{k}={v}" for k, v in params.items()])
    if response_status and response_time:
        logger.info(f"🌐 API CALL: {url} -> status={response_status}, time={response_time:.2f}s")
        logger.debug(f"🌐 API PARAMS: {params_str}")
    else:
...
def list_service_families():
    logger.info("🔧 FUNCTION CALL: list_service_families()")
    ...
    logger.info(f"✅ FUNCTION RESULT: list_service_families -> status=success, count={len(official_service_families)}")
    return result
...
def get_products(service_family, region="westeurope", type="", service_name="", product_name_contains="", limit=0):
    logger.info(f"🔧 FUNCTION CALL: get_products(service_family={repr(service_family)}, region={repr(region)}, type={repr(type)}, service_name={repr(service_name)}, product_name_contains={repr(product_name_contains)}, limit={repr(limit)})")
    ...
    try:
        ...
        logger.info(f"Querying API with filter: {filter_params}")
        ...
        if len(items) == 0:
            logger.warning(f"No products found for the specified criteria: {filter_params}")
            return {
                "product_names": [],
                "count": 0,
                "total_products": 0,
        ...
        while next_page_link and page_count < max_pages:
            logger.info(f"Getting additional page {page_count + 1}: {next_page_link}")
            try:
                ...
                logger.error(f"Error getting additional page: {str(e)}")
                break
        ...
        if product_name_contains:
            ...
            logger.info(f"Filtering: from {len(all_items)} products, {len(filtered_items)} contain '{product_name_contains}'")
        ...
        if limit > 0 and len(product_names) > limit:
            ...
            logger.info(f"Limiting results: showing {len(limited_product_names)} of {len(product_names)} total products")
            was_limited = True
        else:
        ...
        logger.info(f"Retrieved {len(limited_product_names)} unique product names from {len(all_items)} total products across {page_count} pages")
        return {
        ...
        logger.error(f"Error connecting to Azure API: {str(e)}")
        return {
...
def get_service_names(service_family, region="westeurope", max_results=500):
    logger.info(f"🔧 FUNCTION CALL: get_service_names(service_family={repr(service_family)}, region={repr(region)}, max_results={repr(max_results)})")
    ...
    if service_family.lower() == "compute":
        logger.info(f"Using optimized approach for Compute family, limiting to {max_results} results")
        ...
        try:
            # Make the request to the Azure API
            logger.info(f"Querying API with optimized filter: {filter_params}")
            response = requests.get(AZURE_PRICE_API, params=params)
            response.raise_for_status()
            result = response.json()
    ...
    try:
        ...
        if products_result.get("status") == "error" or products_result.get("count", 0) == 0:
            logger.warning(f"No products found for family {service_family}")
            return {
                "service_family": service_family,
                "service_names": [],
                "count": 0,
        ...
        logger.info(f"Querying API to get service names for {service_family}")
...
def get_monthly_cost(product_name, region="westeurope", monthly_hours=730, type="Consumption"):
    logger.info(f"🔧 FUNCTION CALL: get_monthly_cost(product_name={repr(product_name)}, region={repr(region)}, monthly_hours={repr(monthly_hours)}, type={repr(type)})")
    ...
    try:
        # Make the request to the Azure API
        logger.info(f"Querying API to get the price of {product_name} in {region}")
        response = requests.get(AZURE_PRICE_API, params=params)
        response.raise_for_status()
        result = response.json()
...
class HealthEndpoint(HTTPEndpoint):
    ...
    async def get(self, request):
        return JSONResponse({
            "status": "healthy",
            "message": "Azure MCP Pricing Server is running",
            "version": "v3-fixed"
        })
...
class ToolsEndpoint(HTTPEndpoint):
    ...
    """
    async def get(self, request):
        # Return a simplified list of tools instead of using mcp.list_tools() 
        # which might be causing the internal server error
        tools = {
            "tools": [
                {
                    "name": "list_service_families",
                    "description": "List all Azure service families"
                },
                {
                    "name": "get_service_names", 
                    "description": "Get service names within a family"
                },
                {
                    "name": "get_products",
                    "description": "Get products for a specific service in a region"
                },
                {
                    "name": "get_monthly_cost",
                    "description": "Calculate monthly cost for a specific product"
                }
            ]
        }
        return JSONResponse(tools)

# Create the Starlette application with MCP routes
# FastMCP provides an ASGI app through the http_app() method  
app = mcp.http_app()
...
if __name__ == "__main__":
    print("🚀 CONSOLE: Azure Pricing MCP Server starting up...")  # Direct console output
    logger.info(f"🚀 Starting MCP server at http://{settings.MCP_HOST}:{settings.MCP_PORT}")
    logger.info(f"📡 SSE Endpoint: http://{settings.MCP_HOST}:{settings.MCP_PORT}/sse")
    logger.info(f"🔧 Tools Endpoint: http://{settings.MCP_HOST}:{settings.MCP_PORT}/tools")
    logger.info(f"❤️  Health Endpoint: http://{settings.MCP_HOST}:{settings.MCP_PORT}/")
    logger.info(f"🐛 Debug mode: {'ON' if settings.MCP_DEBUG else 'OFF'}")
    logger.info(f"🔄 Auto-reload: {'ENABLED' if settings.MCP_RELOAD else 'DISABLED'}")
    print(f"🔧 CONSOLE: Logger type at startup: {type(logger)}")  # Direct console output
